...
def criar_token(id_usuario, duracao_token=timedelta(days=ACCESS_TOKEN_EXPIRE_MINUTES)):
    ...
    jwt_codificado = jwt.encode(dic_info, SECRET_KEY, ALGORITHM) # type: ignore
...
async def home():
    ...
    """
    return {"mensagem": "Você acessoua rota padrão de autenticação", "autenticação": False}
...
async def login_form(dados_formulario: OAuth2PasswordRequestForm = Depends(), session: Session = Depends(pegar_sessao)):
    usuario = autenticar_usuario(dados_formulario.username, dados_formulario.password, session) # type: ignore
    if not usuario:
        raise HTTPException(status_code=400, detail="Usuário não encontrado ou credenciais Inválidas")
    else:
       access_token = criar_token(usuario.id)
...
async def use_refresh_token(usuario: Usuario = Depends(verificar_token)):
    access_token = criar_token(usuario.id) # type: ignore
    return {
        "Access-Token": access_token,
        "token_type": "Bearer"
    }
